package com.example.coursecatalog.assessments

import androidx.lifecycle.LiveData
import androidx.lifecycle.MediatorLiveData
import androidx.lifecycle.MutableLiveData
import androidx.lifecycle.ViewModel
import com.example.coursecatalog.database.Assessment
import com.example.coursecatalog.database.CatalogDatabaseDao
import com.example.coursecatalog.database.CourseEntity
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.Job
import kotlinx.coroutines.launch

class AssessmentListViewModel(
    private val courseKey: Long = 0L,
    dataSource: CatalogDatabaseDao) : ViewModel() {

    // reference to catalog database dao
    val database = dataSource

    // reference allows access to close coroutines
    private val viewModelJob = Job()

    // run coroutines on ui scope
    private val uiScope = CoroutineScope(Dispatchers.Main + viewModelJob)

    // live data representing term object we're looking at
    private val course = MediatorLiveData<CourseEntity>()

    // get courses associated with this term
    val courses = database.getAssessmentsByCourse(courseKey)

    // getter for term object we're looking at
    fun getCourse() = course

    // add database source to term object we're looking at
    init {
        course.addSource(database.getCourseWithId(courseKey), course::setValue)
    }

    // allows fragment to observe whether to navigate to the term list
    private val _navigateToCourseDetail = MutableLiveData<Long?>()
    val navigateToCourseDetail: LiveData<Long?>
        get() = _navigateToCourseDetail

    // tells app to cancel all coroutines when closing this fragment
    override fun onCleared() {
        super.onCleared()
        viewModelJob.cancel()
    }

    // tells observer it's time to navigate to the term list
    fun onNavigateToCourseDetail() {
        _navigateToCourseDetail.value = courseKey
    }

    // resets back to null so observer knows not to navigate anymore
    fun onCourseDetailNavigated() {
        _navigateToCourseDetail.value = null
    }

    // adds new blank course and navigates to course detail
    fun onAddAssessment() {
        uiScope.launch {
            val newAssessment = Assessment()
            newAssessment.courseId = courseKey
            val assessmentId = database.insert(newAssessment)
            onNavigateToAssessmentDetail(assessmentId)
            onAssessmentDetailNavigated()
        }
    }

    private val _navigateToAssessmentDetail = MutableLiveData<Long>()
    val navigateToAssessmentDetail
        get() = _navigateToAssessmentDetail

    fun onNavigateToAssessmentDetail(assessmentId: Long) {
        _navigateToAssessmentDetail.value = assessmentId
    }

    fun onAssessmentDetailNavigated() {
        _navigateToAssessmentDetail.value = null
    }

}